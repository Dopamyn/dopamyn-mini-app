...
export async function GET(request: NextRequest) {
  try {
    ...
    const sortBy = searchParams.get("sort_by") || "rewards_earned_desc";
    const start = searchParams.get("start") || "0";
    const limit = searchParams.get("limit") || "10";

    // Forward the request to the backend API with the auth header and parameters
    const response = await fetch(
      `${EXTERNAL_API_BASE}/user/quest-earnings?sort_by=${sortBy}&start=${start}&limit=${limit}`,
      {
        headers: {
    ...
    return NextResponse.json(
      { error: error.message || "Failed to fetch quest earnings" },
